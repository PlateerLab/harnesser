...
# 4. 다시 응시 (기록 보존 retake)
at2 = ad.post(f"{API}/attempts/{at['id']}/retake").json()
check("retake starts new attempt", at2.get("status") == "in_progress" and at2["id"] != at["id"], str(at2)[:200])
rrows2 = ad.get(f"{API}/review/attempts").json()
old = next((x for x in rrows2 if x["id"] == at["id"]), None)
check("old attempt preserved+superseded", old is not None and old["superseded"] is True, str(old)[:200])
new = next((x for x in rrows2 if x["id"] == at2["id"]), None)
check("new attempt visible not superseded", new is not None and new["superseded"] is False, str(new)[:150])
rows = ad.get(f"{API}/my/assignments").json()
std2 = next(x for x in rows if x["assessment_id"] == std["assessment_id"])
check("dashboard shows active attempt", std2["attempt_id"] == at2["id"], str(std2["attempt_id"]))
r = ad.post(f"{API}/attempts/{at2['id']}/retake")
check("retake blocked while in_progress", r.status_code == 400, str(r.status_code))
